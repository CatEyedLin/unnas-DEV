...
public class NetworkingTheEasyWay : MonoBehaviour
{
    ...
    private void LobbyManager_OnNetworkMessage(long lobbyId, long userId, byte channelId, byte[] data)
    {
        ...
        if (System.Text.Encoding.UTF8.GetString(data) == "draw")
        {
            //pull card from top of deck | or maybe pull card from random postion in deck

            lobbyManager.SendNetworkMessage(lobby.Id, userId, 0, System.Text.Encoding.UTF8.GetBytes("drawnen = " + new Card().ToString()));
    ...
    {
        //var userId = lobbyManager.GetMemberUserId(lobby.Id, );
        chat.text += "SendNetworkMessage to:" + lobby.OwnerId.ToString() + " in lobby:" + lobby.Id.ToString() + "\n";
        lobbyManager.SendNetworkMessage(lobby.Id, lobby.OwnerId, 0, System.Text.Encoding.UTF8.GetBytes("Draw"));
    }
}
